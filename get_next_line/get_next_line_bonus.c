...
/*   Updated: 2025/01/29 11:45:41 by ael-most         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0 || fd >= 10240)
		return (NULL);
	if (read(fd, 0, 0) < 0)
		return (free_lst(lst[fd]), lst[fd] = NULL, NULL);
	line = NULL;
	read_and_lst(fd, &lst[fd]);
	if (lst[fd] == NULL)
		return (NULL);
	extract_line(lst[fd], &line);
	clean_lst(&lst[fd]);
	if (line == NULL || line[0] == '\0')
		return (free_lst(lst[fd]), lst[fd] = NULL, free(line), NULL);
	return (line);
}
